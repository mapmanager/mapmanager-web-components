...
`setSource()` may be called before connection and is queued. Other methods
require a connected element. Methods that load signal data return promises.
...
payloads are in `CustomEvent.detail` and match the Vue events.
